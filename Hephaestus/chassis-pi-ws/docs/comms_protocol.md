...
# 19 ASRPRO TWEN51 与 Pi 通信协议

## 19.1 协议边界

ASRPRO 不使用 MCU 二进制帧；它通过独立 USB 串口与树莓派通信；默认设备为 `/dev/atlas_asrpro`；默认波特率为 `115200`
...
`HH` 为载荷 UTF-8 字节 XOR；当前版本为 `1`

## 19.3 命令

```text
P2A HELLO firmware_version boot_id
A2P HELLO_ACK boot_id
A2P LISTEN 0|1
A2P SPEAK phrase_id
P2A ACK command
P2A NACK command reason
P2A EVENT ASR intent
P2A EVENT SPEAK_DONE phrase_id
A2P EVENT_ACK event_type
A2P PING timestamp
P2A PONG timestamp
```
...
```text
ASRPRO 上电并周期发送 HELLO
→ Pi atlas_asrpro_bridge 返回 HELLO_ACK
→ Pi 发送 LISTEN,1
→ ASRPRO 返回 ACK
→ MCU AutoPi 手势建立 auto_start_latched
→ 状态机请求 SPEAK transition_complete
→ ASRPRO 播报并发送 SPEAK_DONE
→ 状态机请求 SPEAK voice_prompt
→ ASRPRO 播报并发送 SPEAK_DONE
→ 操作员说 Atlas 启动
→ ASRPRO 发送 EVENT ASR atlas_start
→ 状态机请求 SPEAK autonomous_start
→ 播报完成后进入全自主运输
```

## 19.5 详细适配

完整帧示例；phrase_id 表；intent 表；固件伪代码；重发和去重要求见

```text
docs/ASRPRO_TWEN51适配与烧录说明.md
```
