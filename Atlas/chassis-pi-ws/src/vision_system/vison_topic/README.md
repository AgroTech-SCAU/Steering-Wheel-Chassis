...
当前代码最多取置信度最高的 4 个检测参与角分配，且只使用检测中心的像素坐标。四个目标时，先按 `v` 分成上、下两行，再在每行按 `u` 区分左、右；三个目标时，按 `v` 排序并比较两个相邻纵向间隔，较小间隔对应同一行的两个目标，再根据 `u` 判断另一行的单独目标靠左还是靠右；两个目标时只按 `u` 从左到右编号 `0、1`；一个目标编号 `0`。停止检测时服务返回的坐标数组也使用相同顺序。

重要限制：

- 四个目标位置接近标准四角布局时，编号才具有明确的 TL/TR/BR/BL 含义；
- 三个目标根据纵向间隔识别同一行；目标旋转过大、两行纵向间隔不明显时可能误判；
- 两个目标只能确定左右顺序，编号固定为 `0、1`，不表示真实的上下角身份；
- PnP 模式必须得到完整、顺序正确的四个角点；
- 目标严重旋转、上下行发生交叉或混入误检时，角编号可能不稳定。
...
检查是否有误检、漏检、上下两行发生交叉或多个目标中心过近。两三个目标不能可靠恢复缺失的固定角身份。
