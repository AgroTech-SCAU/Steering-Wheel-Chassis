...
# 树莓派优化模式
ros2 run vison_topic vision_detect_server --rpi --camera 0
        ...
        if resp and resp.success:
            for i in range(resp.count):
                self.get_logger().info(
                    f"[{resp.cls_names[i]}] "
                    f"X={resp.x_mm[i]:.1f}mm Y={resp.y_mm[i]:.1f}mm"
                )
```
...
class DetectionListener(Node):
    ...
    def _on_detections(self, msg: Float32MultiArray):
        ...
        for i in range(count):
            ...
            x_mm   = data[offset + 1]   # X 坐标 (mm)
            y_mm   = data[offset + 2]   # Y 坐标 (mm)

            self.get_logger().info(
                f"检测: cls={cls_id}  X={x_mm:.1f}mm  Y={y_mm:.1f}mm"
            )
```
...
private:
    void on_detections(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
        ...
        for (int i = 0; i < count; ++i) {
            int offset = 1 + i * 3;
            if (offset + 2 >= static_cast<int>(data.size())) break;

            int   cls_id = static_cast<int>(data[offset]);
            float x_mm   = data[offset + 1];     // X 坐标 (mm)
            float y_mm   = data[offset + 2];     // Y 坐标 (mm)

            RCLCPP_INFO(get_logger(),
                "检测: cls=%d  X=%.1fmm  Y=%.1fmm", cls_id, x_mm, y_mm);
        }
    }
...
data[0]          = N     检测目标数量
data[1 + 3×i]    = cls_id   类别 ID  (0=螺丝, 1=齿轮, …)
data[2 + 3×i]    = x_mm     X 坐标  (mm, base_link 坐标系)
data[3 + 3×i]    = y_mm     Y 坐标  (mm, base_link 坐标系)
```

**示例** — 检测到 1 个螺丝 (cls=0, x=-812, y=-1176) + 1 个齿轮 (cls=1, x=-1337, y=-1551)：

```
data = [2.0,  0.0, -811.9, -1175.8,  1.0, -1336.9, -1551.3]
         N    cls   x       y        cls   x        y
              └──── 螺丝 ────┘       └──── 齿轮 ────┘
```
...
float64[] x_mm                      # base_link 系 X 坐标 (mm)
float64[] y_mm                      # base_link 系 Y 坐标 (mm)
