...
class PrepareAction:
    ...
    timeout_s: float = 8.0
    suction_valid: bool = False
    suction_enable: bool = False
...
class VisionPollinationBackend(Node):
    def __init__(self) -> None:
        ...
        self.suction_service = self.declare_parameter('suction_service', '/mcu/set_suction').value
        ...
        self.command_waits_motion = False
    ...
    def load_config(self, path: str) -> None:
        ...
        for name, node in (data.get('prepare_actions') or {}).items():
            ...
            suction_valid = 'suction_enable' in node or 'suction' in node
            suction_enable = bool(node.get('suction_enable', node.get('suction', False)))
            self.prepare_actions[name] = PrepareAction(
                action_type,
                joints,
                float(node.get('speed_rad_s', self.default_speed_rad_s)),
                float(node.get('timeout_s', self.motion_timeout_s)),
                suction_valid,
                suction_enable,
            )
    ...
    def start_current_step(self) -> None:
        ...
        if kind == 'ensure_prepare_pose':
            ...
            self.start_suction_action(step)
        elif kind == 'dwell':
    ...
    def start_suction_action(self, step: dict) -> None:
        ...
        if not self.suction_client.service_is_ready():
            ...
            return
        req = SetBool.Request()
        req.data = bool(step.get('enable', step.get('suction_enable', step.get('suction', False))))
        self.command_future = self.suction_client.call_async(req)
        self.command_waits_motion = False
        self.step_started_at = self.get_clock().now()
        self.message = '发送吸盘打开命令' if req.data else '发送吸盘关闭命令'
    ...
    def start_joint_action(self, action: PrepareAction) -> None:
        ...
        if hasattr(req, 'suction_valid'):
            req.suction_valid = bool(action.suction_valid)
            req.suction_enable = bool(action.suction_enable)
        self.pending_target_joints = np.array(req.joints_rad, dtype=float)
        self.command_future = self.joints_client.call_async(req)
        self.command_waits_motion = True
        self.step_started_at = self.get_clock().now()
    ...
    def start_visual_position(self, step: dict) -> None:
        ...
        self.apply_suction_fields(req, step)
    ...
    def update_running(self) -> None:
        ...
        if self.command_future is not None:
            ...
            self.command_future = None
            if not resp.success:
                self.fail(3009, f'机械臂或吸盘命令被拒绝: {resp.message}')
                return
            if not self.command_waits_motion:
                self.advance_step(f'服务命令完成: {getattr(resp, "message", "ok")}')
                return
            self.step_started_at = self.get_clock().now()
