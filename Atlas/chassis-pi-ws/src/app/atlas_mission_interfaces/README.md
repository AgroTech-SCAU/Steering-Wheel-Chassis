...
### AutonomousTransportStatus.msg

用途

```text
发布全自主运输任务的生命周期、当前阶段、园区分类映射、货物计数和剩余时间
```
...
| 字段 | 含义 |
|---|---|
| `state` | 生命周期状态：等待、预检、运行、结果上报、复位或恢复 |
| `stage` | 中转区播报、语音启动、分拣识别、抓取、导航、投放等任务阶段 |
| `voice_start_received` | 是否收到有效语音启动文本 |
| `sorting_rule_confirmed` | 是否完成园区分类规则确认 |
| `sorting_rule_confidence` | 园区分类规则识别置信度，范围 0.0 到 1.0 |
| `park_1_cargo` | 园区 1 对应的货物类别 |
| `park_2_cargo` | 园区 2 对应的货物类别 |
| `current_cargo` | 当前准备抓取或正在运输的货物类别 |
| `current_waypoint` | 当前导航目标编号 |
| `delivered_total` | 已完成投放数量 |
| `remaining_s` | 全自主运输配置时间预算中的剩余时间 |
| `error_code` | 状态机错误码 |
| `message` | 当前状态说明 |

### ClassifySortingRule.srv

用途

```text
对智能分拣区分类标识进行多次采样，返回齿轮与 T 型螺栓对应的园区映射
```

响应约束

```text
success=true 时 park_1_cargo 与 park_2_cargo 必须分别为 gear 和 t_bolt，且两者不能相同
confidence 的范围为 0.0 到 1.0
```

### ManipulationStatus 的目标计数字段

| 字段 | 含义 |
|---|---|
| `target_count` | 本次任务实际识别并进入动作序列的目标数量 |
| `target_found` | 本次任务是否至少得到一个可执行视觉目标 |

全自主运输状态机使用这两个字段判断视觉抓取任务属于“已处理一个目标”还是“无目标按策略跳过”。固定投放、空动作和无目标任务发布 `target_count=0`。
