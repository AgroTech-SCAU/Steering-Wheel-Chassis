...
在 `mission_route.yaml` 中配置，也可以在 `mission_stack.launch.py` 中通过 launch 参数覆盖：
...
```text
full            使用 Nav2 NavigateToPose 的完整导航后端
pseudo          不依赖地图和定位的伪导航后端，用于安全联调
racom_vision    新增 ONNX 检测模型 + 兼容 DetectCameraTarget 的适配服务
vision_pollination  旧相机目标服务 + 原授粉动作执行器
```

`racom_vision` 并不是重写整套机械臂动作逻辑，而是替换原相机模型服务；动作序列仍复用 `atlas_vision_pollination_backend/config/pollination_actions.yaml`。这样任务状态机、路线 YAML 和机械臂动作模板不需要大范围改动。
...
`mcu_comm_bridge` 现在支持 PI 端条件控制末端吸盘：
...
默认要求 MCU 处于 `AutoPi`，避免手动模式下 PI 误触发末端执行器。动作序列中也可以加入：
...
或者在 `joints_action` / `visual_position` 步骤中加入：

```yaml
suction_enable: true
```

当没有配置 `suction_enable` 时，该动作不会改变吸盘状态。
