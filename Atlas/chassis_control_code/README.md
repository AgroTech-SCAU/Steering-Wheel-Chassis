...
└── src/
    ...
    ├── service/                 # chassis/odom/arm/remote/pc_comms/pi_comms/asr_comms
    │   └── assemble/            # 设备与平台装配
...
delay -> log -> rgb -> imu -> odom -> chassis -> arm -> remote
      -> pc_comms -> pi_comms -> asr_comms -> app_runtime/app_status -> TIM6 500Hz
```
...
| 250 Hz | `odom.process()` |
| 100 Hz | `remote_process()`、`pc_comms_process()`、`pi_comms_process()`、`asr_comms_process()` |
...
注意：HFSM 的原始转移表仍允许 `Manual/Finished -> AutoPi`，但 `app_runtime` 只在 Idle 接受已武装的 ASR 自动启动事件；后续建议收紧底层转移表，形成双重约束
...
- `app_runtime` 只在 `Idle` 接受自动语音门控请求
- `SWD` 处于手动请求时，自动启动不生效
...
- 清除 pending 自动启动事件
- 清除 `voice_gate_armed` 和 pending ASR `AUTO_START` 事件
...
遥控自动手势只请求 MCU 开启一次 ASRPro 语音门控，不直接设置 `auto_start_latched`，也不直接执行 `Idle -> AutoPi`

MCU 仅在以下条件全部满足时发送 `SPK,1\n` 并设置本地 `voice_gate_armed`：
...
遥控自动手势
        ↓
MCU 检查基本启动条件并发送 SPK,1
        ↓
ASRPro 依次播报门控提示
        ↓
用户说“阿特拉斯启动”
        ↓
ASRPro 返回 EVT,AUTO_START
        ↓
MCU 检查 voice_gate_armed，并重新完整检查安全条件
        ↓
auto_start_latched = 1
Idle -> AutoPi
        ↓
MCU best-effort 发送 SPK,2
        ↓
ASRPro 播报“开始执行全自主运输任务”
```

ASRPro 识别成功后只返回 `EVT,AUTO_START`，不会自行宣布任务开始。只有 MCU 完成安全复核并实际进入 `AutoPi` 后才发送 `SPK,2`；发送失败只记录日志，不回滚任务状态

如果发送 `SPK,1` 后 Pi 掉线、chassis/odom 失去 ready、进入 Fault/EStop 或切回 Manual，晚到的 `EVT,AUTO_START` 不会获得自动启动许可，也不会触发 `SPK,2`
...
- `src/service/asr_comms.*`：ASRPro `SPK` 编码、`EVT,AUTO_START` 解析和 pending 事件
- `src/service/remote.*`：遥控状态与边沿事件
- `src/infra/binary_frame.*`：统一线协议

PC/Pi 主协议见 [`../docs/comms_protocol.md`](../docs/comms_protocol.md)，ASRPro ↔ MCU 协议见 [`../docs/asrpro_mcu_protocol.md`](../docs/asrpro_mcu_protocol.md)
