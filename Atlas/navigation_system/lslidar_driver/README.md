...
> **定位**：LSLIDAR 激光雷达 ROS2 驱动节点，负责设备通信与数据解析 | **核心依赖**：`lslidar_msgs`、`rclcpp`、`sensor_msgs`、`pcl_conversions`、`libpcap` | **上游**：雷达硬件（UDP 数据流） | **下游**：`robot_startup`（启动时加载）、Nav2 costmap（消费 `/scan` 话题）
...
| 雷达型号 | **N10P**（单线激光雷达） |
| 通信接口 | **UART（串口）** |
| 配置文件 | `config/lslidar_n10p_uart.yaml` |
| 扫描话题 | `/scan`（sensor_msgs/LaserScan） |
| 坐标系 | `laser_link` |

> **提示**：以下是厂商原始文档，包含完整参数说明、多系列雷达支持和 FAQ；本项目仅使用 N10P UART 模式，其他雷达型号的配置请参考原始文档对应章节。
